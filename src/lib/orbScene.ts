...
export type OrbVisualState = "idle" | "locked" | "tracking" | "holding" | "triggered" | "error";
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  const chromaticShader = {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Push towards a cool blue/cyan holographic tone
        gl_FragColor.rgb = mix(gl_FragColor.rgb, gl_FragColor.rgb * vec3(0.55, 0.95, 1.22), 0.3);
      }
    `,
  };
  ...
  const C_MID = 0x0ea5e9;
  const C_DIM = 0x0369a1;
  ...
  let visualState: OrbVisualState = "idle";
  let energyTarget = 0;
  let energy = 0;
  let pulseStrength = 0;
  const stateColor = new THREE.Color(C_BRIGHT);
  const stateColors: Record<OrbVisualState, THREE.Color> = {
    idle: new THREE.Color(0x075985),
    locked: new THREE.Color(0x2563eb),
    tracking: new THREE.Color(0x38bdf8),
    holding: new THREE.Color(0x7dd3fc),
    triggered: new THREE.Color(0xffffff),
    error: new THREE.Color(0xff2233),
  };
  const stateHaloMat = new THREE.MeshBasicMaterial({
    color: C_BRIGHT,
    transparent: true,
    opacity: 0.04,
  ...
  const stateHalo = new THREE.Mesh(
    new THREE.SphereGeometry(2.24, 36, 24),
    stateHaloMat,
  );
  orbGroup.add(stateHalo);
  ...
  function makeTextSprite(text: string, size = 0.08) {
    ...
    ctx.fillStyle = `rgba(${(35 + Math.random() * 40) | 0}, ${(155 + Math.random() * 65) | 0}, ${(225 + Math.random() * 30) | 0}, ${alpha})`;
  ...
  g.addColorStop(0.5, "rgba(3,105,161,0.15)");
  ...
  function setState(state: OrbVisualState) {
    visualState = state;
  }

  function setEnergy(value: number) {
    energyTarget = THREE.MathUtils.clamp(value, 0, 1);
  }
  ...
  function animate() {
          ...
          : visualState === "holding"
            ...
            : visualState === "tracking"
              ...
              : visualState === "locked"
                ? 0.45
                : 0.7;
    stateColor.lerp(stateColors[visualState], 0.08);
    stateHaloMat.color.copy(stateColor);
    stateHaloMat.opacity = THREE.MathUtils.clamp(
      0.025 + energy * 0.12 + pulseStrength * 0.22,
      0.02,
      0.35,
    );
    stateHalo.scale.setScalar(1 + energy * 0.06 + pulseStrength * 0.18);
    stateHalo.rotation.y -= 0.0025 * stateSpeed;
    stateHalo.rotation.x += 0.0015 * stateSpeed;

    // Outer shell rotation
    outerShell.rotation.y += 0.0015 * stateSpeed;
    outerShell.rotation.x = Math.sin(t * 0.08) * 0.05;

    // Panel group follows shell but with slight offset
    panelGroup.rotation.y += 0.0018 * stateSpeed;
    panelGroup.rotation.x = Math.sin(t * 0.08 + 0.5) * 0.04;

    // Secondary shell counter-rotates slowly
    shell2.rotation.y -= 0.001 * stateSpeed;
    shell2.rotation.z = Math.sin(t * 0.12) * 0.03;

    // Inner core — opposite, faster
    innerCore.rotation.y -= 0.005 * stateSpeed;
    innerCore.rotation.z += 0.002 * stateSpeed;
    innerCore.rotation.x = Math.cos(t * 0.1) * 0.08;

    // Innermost wireframe
    icoWire.rotation.x += 0.008 * stateSpeed;
    icoWire.rotation.y += 0.012 * stateSpeed;
    ...
    const coreScale = 1 + surge + Math.sin(t * 5) * 0.05 + energy * 0.22 + pulseStrength * 0.7;
    ...
    coreSphereMat.opacity = Math.min(0.85, coreOpacity + energy * 0.18 + pulseStrength * 0.25);
    coreSphereMat.color.copy(stateColor);
    glowSphere.scale.setScalar(1 + surge * 0.8);
    glowSphereMat.opacity = Math.max(0, (0.03 + surge * 0.08 + energy * 0.1 + pulseStrength * 0.18) * (1 - fadeOut * 0.9));
    glowSphereMat.color.copy(stateColor);
    // Icosahedron wireframe stays visible even when glow fades
    icoWire.scale.setScalar(1 + surge * 0.6);
    icoWireMat.opacity = Math.min(1, 0.5 + surge * 0.4 + energy * 0.2);
    icoWireMat.color.copy(stateColor);
    ...
    // Bloom pulse
    bloom.strength = 1.25 + Math.sin(t * 0.8) * 0.2 + energy * 0.9 + pulseStrength * 1.4 + (visualState === "error" ? 0.45 : 0);
